...
class PurchaseOrderDetailInvoiceBottomSheet extends StatelessWidget {
  final PurchaseOrderList item;

  const PurchaseOrderDetailInvoiceBottomSheet({super.key, required this.item});
  ...
  Widget _buildHeader(BuildContext context) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          const Text("Detail Pembelian", style: AppTypography.largeBoldBlack),
          IconButton(
            ...
            child: Column(
              children: [
                _buildInfoRow(
                  ...
                  title: formatDateString(item.purchDate.toString()),
                  subtitle: item.farmLocation?.name ?? "-",
                ),
                const Divider(height: 24, color: AppColors.fieldBorder),
                _buildInfoRow(
                  icon: AppImages.icUserTagSvg,
                  title: item.supplierName ?? "-",
                  subtitle: item.supplierAddress ?? "-",
                ),
              ],
            ),
          ),
  ...
  Widget _buildItemDetail(PurchaseOrderDetail orderItem, int index) {
    final isAnimal = orderItem.animalName != null;
    final code = isAnimal
        ? orderItem.animalCode
        : orderItem.feedMedicineCode ?? orderItem.equipmentCode;
    final name = isAnimal
        ? orderItem.animalName
        : orderItem.feedMedicineName ?? orderItem.equipmentName;
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                _buildInfoRow(
                  icon: isAnimal ? AppImages.icNavCow : AppImages.icProduct,
                  title: code ?? "-",
                  subtitle: name ?? "-",
                ),
                const Divider(height: 24, color: AppColors.fieldBorder),
                _buildValueRow(
                  label1: isAnimal ? "Berat Awal" : "Kuantitas",
                  value1: isAnimal
                      ? "${orderItem.initialWeight} kg"
                      : "${orderItem.quantity} unit",
                  label2: "Harga Beli",
                  value2: "Rp ${formatPrice(orderItem.purchPrice)}",
                ),
              ],
            ),
          ),
  ...
  Widget _buildSubItemBox(PurchaseOrderDetail orderItem) {
    final subtotal =
        orderItem.purchPrice * (orderItem.quantity > 0 ? orderItem.quantity : 1);
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Column(
        children: [
          _buildInfoRow(
            ...
            title: "Rp ${formatPrice(subtotal)}",
            subtitle: "Subtotal Item",
          ),
          const Divider(height: 24, color: AppColors.fieldBorder),
          _buildPricingRow(
            "Harga beli",
            "Rp ${formatPrice(orderItem.purchPrice)}",
          ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _buildPricingRow("Jumlah Item", item.details.length.toString()),
          const SizedBox(height: 8),
          _buildPricingRow(
            "Subtotal",
            "Rp ${formatPrice(item.amountPurchased)}",
          ),
          const SizedBox(height: 8),
          _buildPricingRow(
            "Biaya Pengiriman",
