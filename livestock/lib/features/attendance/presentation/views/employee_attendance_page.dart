...
import '../../providers/attendance_form_provider.dart';
...
class _EmployeeAttendancePageState
  ...
  Widget build(BuildContext context) {
    /// init default status hadir (aman via listen)
    ref.listen(employeeListProvider, (prev, next) {
      next.whenOrNull(
        data: (employees) {
          ref.read(attendanceStatusProvider.notifier).init(employees);
        },
      );
    });
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Lakukan Absen"),
          ...
          Expanded(
            child: RefreshIndicator(
              ...
              onRefresh: () async {
                ...
                await ref.refresh(employeeListProvider.future);
  ...
  Widget _summary(List<Employee> employees) {
    ...
    final present = statuses.values.where((v) => v).length;
